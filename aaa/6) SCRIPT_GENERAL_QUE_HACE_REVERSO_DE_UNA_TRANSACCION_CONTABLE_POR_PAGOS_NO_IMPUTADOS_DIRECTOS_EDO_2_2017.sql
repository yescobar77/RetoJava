--6) SCRIPT_GENERAL_QUE_HACE_REVERSO_DE_UNA_TRANSACCION_CONTABLE_POR_PAGOS_NO_IMPUTADOS_SOLO_DIRECTOSEDO_2_2016_ADAPTADO 
--->LEER OBSERVACION

--ojo: enviar a jorge para que lo sustituya en el paquete: REVERSO_PAG_NOIMP_DIRECTOS (el quinto paquete en certificacion - paquetes
-- PK_REVERSO_TRANSACCION_CONT - REVERSO_PAG_NOIMP_DIRECTOS)

--SCRIPT_GENERAL_QUE_HACE_REVERSO_DE_UNA_TRANSACCION_CONTABLE_POR_PAGOS_NO_IMPUTADOS_SOLO_DIRECTOSEDO_2_2016_
--_ADAPTADO_PARA_EJECUTAR_EN_EL_EJERCICIO_EN_QUE_ESTEMOS
declare
...
SELECT  tran.*,fecha_modif        
FROM    
       (SELECT  'neg=conciliacionNoImputado|anho='||P.ANHO||'|orga_id='||P.ORGA_ID||'|pago_id='||P.PAGO_ID||'|tipg_id='||P.TIPG_ID||'' clave,
        ...
        (select t.clave_negocio,t.orga_id,t.anho_presup,
         ...
         from diario_contable dc,detalle_asiento da,asiento a, transaccion t 
	      ...
	      and t.clave_negocio like 'neg=conciliacionNoImputado|anho=%'
              and t.anho=a.anho 
...
--reg.clave_original, reg.orga_id, to_date('20/09/2016','dd/mm/yyyy'), sysdate, 0, 0, 
reg.clave_original, reg.orga_id, reg.fecha_modif, sysdate, 0, 0, 
...
begin  
  ...
  for reg2 in busqueda2 loop
    ...
    values (P_DETASI, P_ASIENTO, reg2.matriz_id, reg2.anho, 
           --to_date('20/09/2016','dd/mm/yyyy'), sysdate, 0, 0, 1, reg2.anho_presup);
    ...
    select diario_contable_seq.nextval, P_DETASI, anho, ctapat_id, 
      -- '20160920', coneco_id, auxiliar, 
         TO_NUMBER(TO_CHAR(REG.FECHA_MODIF,'RRRRMMDD')), coneco_id, auxiliar, 
          -- haber, debe, to_date('20/09/2016','dd/mm/yyyy'), sysdate, 0, 0, 1, anho_presup
             haber, debe, reg.fecha_modif, sysdate, 0, 0, 1, anho_presup
    ...
    select diario_egreso_seq.nextval, P_DETASI, anho, orga_id, capr_id, ffin_id, pluc_id, unor_id, unad_id,
            enre_codigo, 
        --  '20160920', instancia_id, monto * -1, monto_neto * -1, 
            TO_NUMBER(TO_CHAR(REG.FECHA_MODIF,'RRRRMMDD')), instancia_id, monto * -1, monto_neto * -1, 
        -- to_date('20/09/2016','dd/mm/yyyy'), sysdate, 0,
    ...
    select diario_recurso_seq.nextval, P_DETASI, anho, pluc_id, orga_id,  unad_id,
        -- '20160920', instancia_id, monto * -1, 
           TO_NUMBER(TO_CHAR(REG.FECHA_MODIF,'RRRRMMDD')), instancia_id, monto * -1, 
        -- to_date('20/09/2016','dd/mm/yyyy'), sysdate, 0, 0, 1, anho_presup
...
dbms_output.put_line('Termino el proceso REVERSO_PAG_NOIMP_DIRECTOS, transaccion anuladas:'||cantidad);
---hay 5 registros a procesar
